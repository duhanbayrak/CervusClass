...
export type ClassExamRole = 'admin' | 'teacher'

interface StudentExamData {
    studentId: string
    fullName: string
    studentNumber: string
    scores: Record<string, { net?: number }>
    totalNet: number
    rank: number
}
...
async function getClassExamResults(examName: string, classId: string) {
    ...
    return { examName, examDate: results[0].exam_date, className, classId, students }
}

interface ClassExamResultsPageProps {
    readonly examName: string
    readonly classId: string
    readonly role: ClassExamRole
}
...
export async function ClassExamResultsPage({ examName, classId, role }: ClassExamResultsPageProps) {
    ...
    const lowestNet = students.length > 0 ? (students.at(-1)?.totalNet ?? 0) : 0

    const statsData = await getTeacherExamDetailData(decodeURIComponent(examName), classId)

    const backHref = role === 'admin' ? '/admin/exams' : '/teacher/exams'
    const breadcrumb = role === 'admin' ? 'Sınıf Detayı (Admin Görünümü)' : 'Sınıf Detayı'
    const studentDetailBase = role === 'admin'
        ? `/admin/exams/${encodeURIComponent(classData.examName)}/students`
        : `/teacher/exams/${encodeURIComponent(classData.examName)}/students`
    const studentProfileBase = role === 'admin' ? '/admin/students' : '/teacher/students'
    const studentProfileSuffix = role === 'admin' ? '' : '/exams'
                                ...
                                <TableRow key={student.studentId} className="hover:bg-muted/50">
                                    ...
                                    <TableCell>
                                        <div className="flex items-center gap-1">
                                            <Link href={`${studentDetailBase}/${student.studentId}`}>
                                                <Button variant="ghost" size="sm" className="h-8 w-8 p-0" title="Sınav Analizi">
                                                    <ChevronRight className="h-4 w-4" />
                                                </Button>
                                            </Link>
                                            <Link href={`${studentProfileBase}/${student.studentId}${studentProfileSuffix}`}>
                                                <Button variant="ghost" size="sm" className="h-8 w-8 p-0 text-muted-foreground hover:text-primary" title="Öğrenci Profili">
