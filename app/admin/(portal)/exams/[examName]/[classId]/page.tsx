...
async function getClassExamResults(examName: string, classId: string) {
    ...
    // Admin context - works because of RLS fix (view_org_profiles)
...
export default async function AdminClassExamDetailPage({
    ...
    // Grafik verilerini çek - Reusing teacher logic as it's purely organization-scoped
        ...
        <div className="container py-8 max-w-7xl mx-auto space-y-6">
            ...
            <div className="flex items-center gap-4">
                ...
                <div className="flex items-center gap-2 text-muted-foreground">
                    ...
                    <span className="text-sm font-medium">Sınıf Detayı (Admin Görünümü)</span>
                </div>
            </div>
                                ...
                                <TableRow key={student.studentId} className="hover:bg-muted/50">
                                    ...
                                    <TableCell>
                                        <div className="flex items-center gap-1">
                                            {/* Link updated for Admin */}
                                            <Link href={`/admin/exams/${encodeURIComponent(classData.examName)}/students/${student.studentId}`}>
                                                <Button variant="ghost" size="sm" className="h-8 w-8 p-0" title="Sınav Analizi">
                                                    <ChevronRight className="h-4 w-4" />
                                                </Button>
                                            </Link>
                                            {/* Link updated: Admin -> Students -> [id] -> Exams? If that exists. If not, maybe just disable or point to student profile if known. 
                                                Checking: /admin/students/[studentId] usually exists. I'll point to that if possible, or omit if unsure.
                                                Actually, let's assume /admin/students/[studentId] exists.
                                            */}
